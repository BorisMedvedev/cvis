...
function validateSelection() {
  const selectedOption = document.querySelector(
    `#question${currentQuestionIndex + 1} .option.selected`
  );
...
function validateWeightHeightInputs() {
  ...
  nextButtons[currentQuestionIndex].disabled = !(
...
nextButtons.forEach((button, index) => {
  button.addEventListener('click', function () {
    if (currentQuestionIndex === numberButtons) {
...
document
  ...
  .addEventListener('input', validateWeightHeightInputs);
document
...
const calculateBMI = () => {
  if (weight > 0 && height > 0) {
    ...
    document.getElementById('bmi-category').textContent =
      'Данные некорректны. Проверьте ввод.';
